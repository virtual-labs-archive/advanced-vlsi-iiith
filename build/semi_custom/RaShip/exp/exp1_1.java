...
//                                      g2d.drawString("Volt",  10 , 360 );
  ...
  //                              end_index++;
		...
		public MyPanel()
		{
			...
			leftPanel.setMinimumSize(new Dimension(1000,1200)); // for fixing size
...
//                      bottom.add(h );
		...
		public class Circuit extends JPanel
		{
			...
			public void paint(Graphics g)
			{
				/*int i,j;
				String data;
				int local_total_comp = 0,local_img_no = -1,local_x,local_y;
				Graphics2D g2d = (Graphics2D)g;
			...
			///	File f = new File("file.txt");
				...
				local_total_comp  = Integer.parseInt(bf.readLine());
				System.out.println("Hello"+local_total_comp);
				
				for ( i = 0; i < local_total_comp ; i++ )
				{
					local_img_no   = Integer.parseInt(bf.readLine());
					local_x   = Integer.parseInt(bf.readLine());
					local_y   = Integer.parseInt(bf.readLine());*
						if ( local_img_no == 1)
						{
							workPanel.draw_cmos(g2d , local_x  , local_y , 60 , 0);
						//	g.setColor(Color.yellow);
						//	g.drawString(comp_str[1] , comp_node[i].node_x -10 , comp_node[i].node_y );
						}
						...
						else if ( comp_node[i].img_no == 7)
						{
							...
							g.drawString(comp_str[7] , comp_node[i].node_x + -10 , comp_node[i].node_y + 10 );
                                                 ...
                                                 {
                                                         draw_or(g2d , comp_node[i].node_x  , comp_node[i].node_y , 30, comp_node[i].angle);
                                                       g.setColor(Color.yellow);
		 ...
		 public boolean circuit_check()
                 {
                         ...
                         int check_points_value[][] = new int[11][1000] ; // each index will store corresponding  component for points of (comp point no -> index )
	...
	/*		else if (e.getSource() == img_button1[4] ) // Input 
		...
		public  class node
		{
			...
			public void update_end_points_mat(int img)
			{
				...
				else if (  img==4 || img == 10   ) // ground , VDD , INPUT , OUTPUT
                                  {
...
//					a = width ; b= 0 ; c = width ;d = height ; e = 0 ; f = height / 2 ;
				...
				else if ( img == 2 || img == 9 ) //or//terminal
                                 {
                                         count_end_points = 3;
                                          int a , b ,c,d,e,f ;
			...
			public void mouseClicked(MouseEvent m)
			{
				 ...
				 if(img_button_pressed!=-1)
				{
					...
					if(img_button_pressed==1) //|| img_button_pressed==7)
					{
						...
						comp_node[total_comp] = new node((work_x / 15)*15 , (work_y / 15 )*15 , img_button_pressed , 15 * 4 , 4 * 15,pmos_count);
						//	pmos_count++;
							hadder_count++;
				...
				for ( i = 0; i < local_total_comp ; i++ )
				{
					...
					/*	else if ( local_img_no == 3)
						...
						//	g.drawString(comp_str[7] , comp_node[i].node_x + -10 , comp_node[i].node_y + 10 );
                                                 ...
                                                 {
                                                         draw_output(g2d , local_x  , local_y , 15, local_angle);
                                                        // g.setColor(Color.yellow);
                                                        // g.drawString(comp_str[9] , comp_node[i].node_x + 30 , comp_node[i].node_y + 10 );
                                                 }
...
//				g.drawRect( x , y , 2*width , 4*width);
 ...
 //                              g.drawRect( x , y , 2*width , 2*width);
			...
			void draw_output(Graphics2D g , int x , int y , int width , double angle)
			{
				...
				g.rotate(angle , x , y);
				g.setColor(Color.yellow);
				g.drawString("O", x  + width / 2, y+ width - 5);

...
public class exp1 extends JApplet
{
	...
	public class MyPanel  extends JPanel  implements ActionListener//MouseListener,MouseMotionListener
	{
		...
		double scale_x = 0.5 ;  // scaling of work pannel 
		double scale_y = 0.5 ;
		...
		int	prev_xpos = -1;
		int	prev_ypos = -1;
		int hadder_delete_count=0;
		int [] hadder_delete=new int[200];
...
//
		...
		WorkPanel workPanel = new WorkPanel();
		 ...
		 public class graph extends JPanel   implements MouseMotionListener,MouseListener
			 ...
			 int xpos,ypos,ycord;
                        int[][] A = new int[numberofBits][64] ;
                        int[][] B = new int[numberofBits][64] ;
                        int[] C_in = new int[64] ;
                        int[][] C_out = new int[numberofBits][64] ;
                        int[][] S = new int[numberofBits][64] ;
			...
			public void make_grap()
			{
			 ...
			 public void make_graph(String file)
				 ...
				 g2d.setTransform(tx);
					...
					/*	if(A[0][i]==1 && B[i]==0 && C_in[i]==0 )
						{
							S[i]=1;
						}
						else if(A[0][i]==0 && B[i]==1 && C_in[i]==0 )
						{
							S[i]=1;
						}
						else if(A[0][i]==0 && B[i]==0 && C_in[i]==1 )
						{
							S[i]=1;
						}
						else if(A[0][i]==1 && B[i]==1 && C_in[i]==1 )
						{
							S[i]=1;
						}
						else
...
/*					for(i=0;i<64;i++)
					{
						if(A[0][i]==1 && B[i]==1 && C_in[i]==0 )
						{
							C_out[i]=1;
						}
						else if(A[0][i]==0 && B[i]==1 && C_in[i]==1 )
						{
							C_out[i]=1;
						}
						else if(A[0][i]==1 && B[i]==0 && C_in[i]==1 )
						{
							C_out[i]=1;
						}
						else if(A[0][i]==1 && B[i]==1 && C_in[i]==1 )
...
//			leftPanel.add(leftSplitPane, BorderLayout.CENTER);
		//	add(leftSplitPane, BorderLayout.CENTER);
			...
			for ( i = 1 ; i <= 4 ; i ++ )
			{
				...
				{
					System.err.println("Couldn't find file: "+ i );
					icon[i] =  null;
				}
			...
			{
				System.err.println("Couldn't find file: 0" );
				icon[0] =  null;
			}
			...
			add(splitPane, BorderLayout.CENTER);*/
			  ...
			  add(topPanel , BorderLayout.NORTH);
                        ...
                        JLabel heading = new JLabel (  "<html><FONT COLOR=WHITE SIZE=18 ><B>Ripple Carry Adder</B></FONT></html>", JLabel.CENTER);
...
/*                        java.net.URL imgURL = getClass().getResource("simulate1.png");
			...
			String[] input = {"4","8","12"};
			input_list = new JComboBox(input);
			input_list.setSelectedIndex(0);
                        simulate_button = new JButton("Schematic Diagram" , icon_simulate );
//                        icon_simulate.setImageObserver(simulate_button);
                        gate_button = new JButton("Gate Level Diagram" , icon_simulate );
  //                      icon_simulate.setImageObserver(gate_button);
    ...
    //                    icon_graph.setImageObserver(graph_button);
			...
			headButton.add(input_list);
			headButton.add(gate_button);
			headButton.add(simulate_button );
                   ...
                   //     headButton.add(exp_list );
                     //   headButton.add(layout_button );
                        ...
                        gate_button.addActionListener(this);
			input_list.addActionListener(this);
...
//                      bottom.add(h );
	...
	local_endpoint = check_points_value[i][l];
		...
		{
			if(e.getSource() == input_list)
			{
				JComboBox sel = (JComboBox)e.getSource();
				numberofBits =  (4*sel.getSelectedIndex())+4;
				scale_x = 0.20001;
				scale_y = 0.20001;
				simulate_button_pressed  =  -1;
				gate_button_pressed  =  -1;
//
				workPanel.repaint();
			}
			...
			//	System.out.println("Simulated");
				scale_x = 0.20001;
				scale_y = 0.20001;
				trans_x = 0 ;  // tanslating of work pannel 
				trans_y = 0 ;
				workPanel.repaint();
			...
			/*	}
			 ...
			 else if(e.getSource() ==  gate_button )
			  {
					gate_button_pressed  =  1;
					simulate_button_pressed  =  -1;
					trans_x = 0 ;  // tanslating of work pannel 
			...
			else if(true || simulate_button_pressed != 1)
			{
		...
		public  class node
		{
			...
			public node (int x , int y , int no , int w , int h,int number)
			{
				...
				{
					int i;
					if(hadder_delete_count!=0)
					{
					for(i=0;i<hadder_delete_count;i++)
					{
						if(hadder_delete[i]==-1)
							continue;
						node_number=hadder_delete[i];
						hadder_delete[i]=-1;
						break;
					}
					}
				...
				if ( img == 1) // Halfadder
				{
					...
					for ( int k = 0 ; k < 4 ; k ++ )
					{
						for ( int i = end_pointsX[k] +8 ; i > end_pointsX[k] -7 && i>0; i -- )
						{
							for ( int j = end_pointsY[k] +8 ; j > end_pointsY[k] -7 && j>0; j -- )
							{
								end_points_mat[i][j] = (((node_number)*4))+k ;         //total no. of Hadder = 2
							}
						}
					}
				}
...
//                                          a = width / 2 ; b= 0 ;
 ...
 725                                         }*/
                                          ...
                                          {
                                                  hadder_count--;
							hadder_delete[hadder_delete_count++]=comp_node[node_index].node_number;
                                                  //pmos_count--;
  ...
  //                                      work_panel_repaint();
		...
		public class WorkPanel extends JPanel implements MouseMotionListener,MouseListener,MouseWheelListener
		{
//				int scale = 1;
			public WorkPanel()
			{
  ...
  //                                      wire[total_wire-1 ].update2((work_x/20)*20 , (work_y/20)*20);
			...
			public void mouseDragged(MouseEvent me)
			{
				int i,j;
                                  work_x = me.getX();
                                  work_y = me.getY();
					if((simulate_button_pressed == 1 || gate_button_pressed == 1) && prev_work_x != -1 && prev_work_y != -1)
					{
						trans_x = trans_x + work_x - prev_work_x;
						trans_y = trans_y + work_y - prev_work_y;
						
					}
					else if(simulate_button_pressed == -1)
					{
					if(work_x<15)
						work_x=15;
					if(work_y<15)
			...
			if(m.getScrollType() == MouseWheelEvent.WHEEL_UNIT_SCROLL) {
				   ...
				   scale_x += (.1 * m.getWheelRotation());
				   scale_x = Math.max(0.00001, scale_x); 
				   scale_y += (.1 * m.getWheelRotation());
				   scale_y = Math.max(0.00001, scale_y); 
				 																				repaint();
				 																		      }
			...
			public void mouseClicked(MouseEvent m)
			{
				...
				if(img_button_pressed==-1)
				{
					if(simulate_button_pressed == 1)
					{
						//scale_x = scale_x + 0.1;
					        //scale_y = scale_y + 0.1;
				//		trans_x = trans_x + 400 - work_x;
				//		trans_y = trans_y + 250 - work_y;
				//		repaint();
				 ...
				 if(img_button_pressed!=-1)
				{
					...
					if(img_button_pressed==1)
					{
						if(hadder_count >= 2)
						{
                                                       JOptionPane.showMessageDialog(null, "Circuit Can have maximum of 2 Half Adder ");
						}
						else
						{
				
						comp_node[total_comp] = new node((work_x / 15)*15 , (work_y / 15 )*15 , img_button_pressed , 15 * 4 , 4 * 15,hadder_count);
						//	pmos_count++;
							hadder_count++;
				...
				total_comp++;
						...
						if(input_count>=3)
						{
                                                       JOptionPane.showMessageDialog(null, "Circuit Can have maximum  3 inputs ");
						}
						else
						{
				...
				total_comp++;
						...
						if(output_count>=2)
						{
                                                       JOptionPane.showMessageDialog(null, "Circuit Can have maximum 2 outputs ");
						}
						else
						{
...
1146                                         }*/
			...
			//	g2d.setColor(Color.black);
				...
				if(simulate_button_pressed == 1 || gate_button_pressed == 1)
				{
                                String fileToRead;
				int sepration;
				if (simulate_button_pressed == 1)
				{
                                	fileToRead ="txt/circuit.txt";
					sepration = 1000;
					
				}
				else
				{
					fileToRead = "txt/gate.txt";
					sepration = 500;
				}
//				numberofBits = 4;
				int loop = 0;
				for(loop = 0 ; loop < numberofBits ; loop++)
				{
				// g2d.scale(scale_x , scale_y);
 				 AffineTransform tx = new AffineTransform();
 				 AffineTransform oldtx = new AffineTransform();
				 oldtx = g2d.getTransform();
 				 tx.translate(trans_x,trans_y);
 				 tx.scale(scale_x,scale_y);
 				 tx.translate((sepration*loop),0);
				 g2d.setTransform(tx);
			...
			//	DataInputStream dis = new DataInputStream(bis);
				...
				for ( i = 0; i < local_total_comp ; i++ )
				{
					...
					if(line != null)
						...
						{
							draw_xor(g2d , local_x  , local_y , 15, local_angle);
							g.setColor(Color.yellow);
							//g.drawString(comp_str[1] , comp_node[i].node_x -10 , comp_node[i].node_y + 10 );
						}
						else if ( local_img_no == 12)
						{
							draw_and(g2d , local_x  , local_y , 15 , local_angle);
							g.setColor(Color.yellow);
							//g.drawString(comp_str[1] , comp_node[i].node_x -10 , comp_node[i].node_y + 10 );
						}
						else if ( local_img_no == 17)
						{
							draw_or(g2d , local_x  , local_y , 15 , local_angle);
							g.setColor(Color.yellow);
							//g.drawString(comp_str[1] , comp_node[i].node_x -10 , comp_node[i].node_y + 10 );
						}
						...
						//	g.drawString(comp_str[7] , comp_node[i].node_x + -10 , comp_node[i].node_y + 10 );
							...
							g.drawString("VDD" , local_x + 30 , local_y + 10 );
                                                 ...
                                                 else if ( local_img_no == 4 || local_img_no == 14)
                                                 {
                                                         draw_input(g2d , local_x  , local_y , 15, local_angle);
                                                        // g.setColor(Color.yellow);
                                                        // g.drawString(comp_str[9] , comp_node[i].node_x + 30 , comp_node[i].node_y + 10 );
                                                 }
                                                 else if ( local_img_no == 10 || local_img_no == 20)
                                                 {
				 ...
				 if(loop == (numberofBits - 1))
					local_total_wire = local_total_wire - 1;
...
//				 			local_end_index   = Integer.parseInt(line.trim());
				...
				else if (simulate_button_pressed == -1 && gate_button_pressed == -1)
				{
				}
				else
				{
				for ( i = 0; i < total_comp ; i++ )
				{
					if ( comp_node[i].del != true )
					{
						...
						{
							draw_hadder(g2d , comp_node[i].node_x  , comp_node[i].node_y , 60 , comp_node[i].angle);
							g.setColor(Color.black);
							g.drawString(comp_str[1] , comp_node[i].node_x   , comp_node[i].node_y + 15);
						}
			...
			void draw_hadder(Graphics2D g , int x , int y , int width , double angle )
                         {
